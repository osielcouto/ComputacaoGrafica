...
function main() {
  ...
  var light = initDefaultLighting(scene, new THREE.Vector3(11, 36, 25));
  ...
  var mountain = textureLoader.load('../assets/textures/montanha.jpg');
  
  var groundPlane = createGroundPlane(500, 500); // width and height
  groundPlane.rotateX(degreesToRadians(-90));
  groundPlane.translateZ(-4.07);
  groundPlane.receiveShadow = true;
  groundPlane.material.map = grass;
  groundPlane.material.map.repeat.set(1000,1000);
  ...
  const material = new THREE.MeshBasicMaterial( { map: mountain,side: THREE.BackSide} );
  const skybox = new THREE.Mesh( geometry, material );
  skybox.translateY(240);
  scene.add( skybox );
  //Forma 2 de criar skybox
  //var geometry = new THREE.BoxBufferGeometry( 500, 500, 500 );
  //var material = new THREE.MeshBasicMaterial( { map: mountain,side: THREE.BackSide } );
